...
- Scope: tooling; artifact-optimization; README, docsync and formatting; Removed obsolete API client logic and complex unit tests in favor of a clean, minimal wrapper; implement e2e tests playwright; fix e2e implementation and package config; add multi-open popup with side panel and tab options; add svg icons to popup buttons; sync release-signal-config with current repository structure; fix CI playwright setup and reporting; Bump node package to "@types/chrome": "^0.1.42"; refactor workflow tests; release signal check ON; fix release signal autofix feedback
